...
import { fetchMoodHistory } from './moodStorage';
import '../App.css';
...
const MoodDashboard = () => {
  ...
  const loadData = async () => {
    ...
    if (rawHistory.length > 0) {
      ...
      setTotalMoods(chartData.reduce((sum, item) => sum + item.value, 0));
    } else {
      setMoodData([]);
      setTotalMoods(0);
    }
  ...
  const handleClearHistory = () => {
    if (window.confirm("Are you sure you want to reset your local vibe history?")) {
      localStorage.removeItem('sonic_mood_history');
      loadData();
    }
  };
  ...
  return (
    <div className="dashboard-container wide">
      <div className="dashboard-header-row">
        <h2>Your Vibe History</h2>
        <button className="reset-btn" onClick={handleClearHistory}>Reset Local Data</button>
      </div>
      
      <div className="stat-cards">
        <div className="stat-card">
          <span className="stat-label">Dominant Vibe</span>
          <span className="stat-value" style={{ color: MOOD_COLORS[topVibe] || '#fff' }}>
            {topVibe}
          </span>
        </div>
        <div className="stat-card">
          <span className="stat-label">Total Moods Logged</span>
        ...
        <div className="dashboard-section">
          ...
          <div className="mood-bars-wrapper">
            {moodData.map((item, index) => {
              const percentage = totalMoods > 0 ? Math.round((item.value / totalMoods) * 100) : 0;
              const barColor = MOOD_COLORS[item.name] || '#A78BFA'; 
          ...
          <div className="recent-logs-wrapper">
            {recentLogs.map((log, index) => (
              <div key={index} className="log-item">
                <div className="log-icon" style={{ backgroundColor: MOOD_COLORS[log.mood] || '#555' }}></div>
                <div className="log-details">
                  <span className="log-mood">Felt <strong>{log.mood}</strong></span>
                  <span className="log-time">Today at {formatTime(log.date)}</span>
                </div>
              </div>
            ))}
          </div>
