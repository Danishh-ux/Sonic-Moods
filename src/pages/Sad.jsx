...
const Sad = () => {
  ...
  useEffect(() => {
    ...
    const fetchSadPlaylist = async () => {
      try {
        ...
        console.error("Error fetching sad playlist:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="app-container theme-sad" style={{ display: 'block', paddingTop: '100px', paddingBottom: '50px' }}>
      <button className="back-btn" onClick={() => navigate('/')}>← Back to Moods</button>
      <div className="playlist-header">
        <h1>🌧️ Melancholy Frequency</h1>
        <p>Slow, acoustic tracks for deep thoughts</p>
      </div>
