...
const Angry = () => {
  ...
  useEffect(() => {
    // 1. Fetch iTunes songs (Industrial/Rock)
    const fetchAngryPlaylist = async () => {
      try {
        ...
        console.error("Error fetching angry playlist:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="app-container theme-angry" style={{ display: 'block', paddingTop: '100px', paddingBottom: '50px' }}>
      <button className="back-btn" onClick={() => navigate('/')}>← Back to Moods</button>
      <div className="playlist-header">
        <h1>🌩️ Distorted Frequency</h1>
        <p>High-energy tracks for raw intensity</p>
      </div>
